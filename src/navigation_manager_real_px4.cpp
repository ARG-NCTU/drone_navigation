...
using namespace std; 
// For real px4 we don't have speed control with joystick so we need to careful about the navigation function first will keep send speed 0, and the localization control doesn't start before recieve the first subgoal waypoint. 
class Navigation{
    private:
        ros::NodeHandle n;
...
void Navigation::navigation() {
    ros::Rate rate(30);
    marginCheck();
    if (!twistCheck() && one_time) { // No human control

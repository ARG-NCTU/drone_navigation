...
using namespace std; 
// remove position keeping part (ardupilot can automatically maintain the last position in guided mode)
// add speed publisher to the ardupilot via mavros (avoid sending velocity and position command at the same time)
class Navigation{
    private:
        ros::NodeHandle n;
...
void Navigation::navigation() {
    ros::Rate rate(30);
    marginCheck();
    if (!twistCheck() && one_time) { // No human control and any subgoal is recieved
        ...
        // else if{ //position keeping in simulation offboard mode (For real drone, ardupilot will maintain last position for guided mode) 
